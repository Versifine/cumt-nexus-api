...
- 可选 Bearer 的公开读取接口：无 `Authorization` 时按匿名读取且 `my_vote=0`/权限对象为匿名态；有合法 Bearer 时返回当前用户 `my_vote` 和 `viewer_permissions`；有格式错误、过期或签名错误的 Bearer 时仍返回 `unauthenticated`。
