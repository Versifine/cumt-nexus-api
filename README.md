...
阶段：`阶段 27 API 请求必填字段契约校验已完成`

代码已完成阶段 1 认证与用户基础闭环、阶段 2 社区申请与审批闭环、阶段 3 帖子发布和读取闭环、阶段 4 评论发布和读取闭环、阶段 5 完整真实冒烟、阶段 6 全站最新帖子流 + 帖子 upvote/downvote 基础、阶段 7 轻量举报与平台 staff 移除内容闭环、阶段 8 审核台最小闭环、阶段 9 hot feed / 内容分发闭环、阶段 10 审核台增强闭环、阶段 11 搜索闭环、阶段 12 通知闭环、阶段 13 内容系统增强闭环、阶段 14 内容编辑与删除闭环、阶段 15 R2 真实凭据 smoke 工具、阶段 16 工程验收入口、阶段 17 HTTP API 契约快照、阶段 18 配置契约清单校验、阶段 19 migration 契约与清单校验、阶段 20 配置语义契约校验、阶段 21 配置加载运行时契约测试、阶段 22 HTTP API request/response schema 契约快照和字段清单校验、阶段 23 HTTP 错误码、HTTP 状态码和错误响应形状契约校验、阶段 24 API schema 路由映射校验、阶段 25 API 认证边界契约校验、阶段 26 API 查询参数契约校验，以及阶段 27 API 请求必填字段契约校验。
...
阶段 27 已完成：补强 API schema 契约校验脚本，确保 `docs/internal/architecture/http-api-schema.md` 的请求必填字段清单与 handler request struct 中的 `binding:"required"` 保持同步。本阶段不新增业务接口，不改变请求校验语义或响应格式。
...
- HTTP API schema 字段清单、路由映射与请求必填字段校验 `scripts/verify-api-schema-doc.ps1`
...
该脚本会依次执行 API 路由/认证边界/查询参数契约校验、API schema 字段/路由/必填字段契约校验、HTTP 错误契约校验、配置清单契约校验、配置语义契约校验、migration 契约校验、测试、构建、migration、Stage 13 内容系统 smoke、Stage 14 内容生命周期 smoke 和 Stage 15 R2 smoke/凭据门禁。默认 `-R2Mode SkipWhenMissing`：没有 R2 dev bucket 凭据时只验证 skipped 分支；如果当前环境或 `.env` 中存在 R2 dev bucket 凭据，则会执行真实 R2 上传并在 dev bucket 留下测试对象。
...
该脚本会扫描 delivery 层 handler JSON struct，并与 `docs/internal/architecture/http-api-schema.md` 的 package、Go type、JSON 字段清单和请求必填字段清单比对；同时读取 `docs/internal/architecture/http-api-contract.md` 的路由表，校验 schema 文档中的接口映射覆盖当前路由、没有过期路由、schema 引用真实存在，并约束成功状态码为 `200/201/204`。请求必填字段只来自 handler request struct tag 中显式存在的 `binding:"required"`；它不生成 OpenAPI，不校验完整业务枚举、数值范围或错误消息全文。
...
- `docs/internal/architecture/http-api-schema.md`：当前 HTTP API request/response schema、接口 schema 映射、handler JSON 字段清单和请求必填字段清单快照
...
- 阶段 27 只补 API 请求必填字段契约校验；不新增业务接口，不改变请求校验语义或成功/错误响应格式。
